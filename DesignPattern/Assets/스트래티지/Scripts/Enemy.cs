...
{
    private float _amplitude = 2f; // 진폭, 강도, 넓이
    private float _frequency = 2f; // 빈도
...
public class CircularMovement : IMovementStrategy
{
    ...
    public void Move(Transform transform, float speed)
    {
        ...
        float x = _center.x + Mathf.Cos(_angle * Mathf.Deg2Rad) * _radius; // 센터 중심으로 빙빙
        float z = _center.z + Mathf.Sin(_angle * Mathf.Deg2Rad) * _radius; // 센터 중심으로 빙빙
